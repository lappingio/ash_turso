...
defmodule AshTurso.DataLayer do
  @index %Spark.Dsl.Entity{
    ...
    target: AshTurso.CustomIndex,
    schema: AshTurso.CustomIndex.schema(),
    transform: {AshTurso.CustomIndex, :transform, []},
    args: [:fields]
  }
  ...
  @statement %Spark.Dsl.Entity{
    ...
    target: AshTurso.Statement,
    schema: AshTurso.Statement.schema(),
    args: [:name]
  }
  ...
  @reference %Spark.Dsl.Entity{
    ...
    args: [:relationship],
    target: AshTurso.Reference,
    schema: AshTurso.Reference.schema()
  }
  ...
  @turso %Spark.Dsl.Section{
    name: :turso,
    describe: """
    Turso data layer configuration
    """,
    sections: [
      @custom_indexes,
      @custom_statements,
    ...
    schema: [
      repo: [
        ...
        required: true,
        doc:
          "The repo that will be used to fetch your data. See the `AshTurso.Repo` documentation for more. Can also be a function that takes a resource and a type `:read | :mutate` and returns the repo."
      ],
      migrate?: [
        type: :boolean,
        default: true,
  ...
  @moduledoc """
  A Turso/libSQL data layer that leverages Ecto and the ecto_libsql adapter.
  """

  use Spark.Dsl.Extension,
    sections: @sections,
    transformers: [
      AshTurso.Transformers.ValidateReferences,
      AshTurso.Transformers.VerifyRepo,
      AshTurso.Transformers.EnsureTableOrPolymorphic
    ]

  def migrate(args) do
    # TODO: take args that we care about
    Mix.Task.run("ash_turso.migrate", args)
  end
  ...
  def rollback(args) do
    ...
    for repo <- repos do
      ...
      migrations_path = AshTurso.Mix.Helpers.migrations_path([], repo)
      ...
      if to do
        Mix.Task.run("ash_turso.rollback", args ++ ["-r", inspect(repo), "--to", to_string(to)])
        Mix.Task.reenable("ash_turso.rollback")
      end
    end
  end
  ...
  if Code.ensure_loaded?(Igniter) do
    def install(igniter, module, Ash.Resource, _path, argv) do
      ...
      igniter
      |> Spark.Igniter.set_option(module, [:turso, :table], table_name)
      |> Spark.Igniter.set_option(module, [:turso, :repo], repo)
    end
  ...
  def codegen(args) do
    Mix.Task.reenable("ash_turso.generate_migrations")
    Mix.Task.run("ash_turso.generate_migrations", args)
  end

  def setup(args) do
    # TODO: take args that we care about
    Mix.Task.run("ash_turso.create", args)
    Mix.Task.run("ash_turso.migrate", args)
  end

  def tear_down(args) do
    # TODO: take args that we care about
    Mix.Task.run("ash_turso.drop", args)
  end
  ...
  def can?(resource, {:join, other_resource}) do
    ...
    data_layer == other_data_layer and
      AshTurso.DataLayer.Info.repo(resource) == AshTurso.DataLayer.Info.repo(other_resource)
  end
  ...
  def can?(_, {:filter_relationship, %{manual: {module, _}}}) do
    Spark.implements_behaviour?(module, AshTurso.ManualRelationship)
  end
  ...
  def run_query(query, resource) do
    ...
    case with_sort_applied do
      ...
      {:ok, query} ->
        ...
        if AshTurso.DataLayer.Info.polymorphic?(resource) && no_table?(query) do
          raise_table_error!(resource, :read)
        else
          repo = dynamic_repo(resource, query)

          opts =
            AshSql.repo_opts(repo, AshTurso.SqlImplementation, nil, nil, resource)
  ...
  def functions(_resource) do
    [
      ...
      AshTurso.Functions.ILike
    ]
  end

  @impl true
  def resource_to_query(resource, _) do
    from(row in {AshTurso.DataLayer.Info.table(resource) || "", resource}, [])
  end
  ...
  def bulk_create(resource, stream, options) do
    ...
    |> Enum.reduce_while({:ok, []}, fn {_, changesets}, {:ok, acc} ->
      repo = AshSql.dynamic_repo(resource, AshTurso.SqlImplementation, Enum.at(changesets, 0))
      opts = AshSql.repo_opts(repo, AshTurso.SqlImplementation, nil, options[:tenant], resource)
      ...
      opts =
        if options[:upsert?] do
          ...
          query =
            query
            |> AshSql.Bindings.default_bindings(resource, AshTurso.SqlImplementation)
  ...
  defp set_table(record, changeset, operation, table_error?) do
    if AshTurso.DataLayer.Info.polymorphic?(record.__struct__) do
      table =
        changeset.context[:data_layer][:table] ||
          AshTurso.DataLayer.Info.table(record.__struct__)
  ...
  defp add_exclusion_constraints(changeset, resource) do
    resource
    |> AshTurso.DataLayer.Info.exclusion_constraint_names()
  ...
  defp add_related_foreign_key_constraints(changeset, resource) do
    ...
    |> Enum.reduce(changeset, fn %{
                                 ...
                                 changeset ->
      case AshTurso.DataLayer.Info.reference(resource, relationship_name) do
        ...
        _ ->
          Ecto.Changeset.foreign_key_constraint(changeset, destination_attribute,
            name: "#{AshTurso.DataLayer.Info.table(source)}_#{source_attribute}_fkey",
            message: "would leave records behind"
          )
      end
    end)
  ...
  defp add_configured_foreign_key_constraints(changeset, resource) do
    resource
    |> AshTurso.DataLayer.Info.foreign_key_names()
    |> case do
  ...
  defp add_unique_indexes(changeset, resource, ash_changeset) do
    changeset =
      ...
      |> Enum.reduce(changeset, fn identity, changeset ->
        name =
          AshTurso.DataLayer.Info.identity_index_names(resource)[identity.name] ||
    ...
    names =
      resource
      |> AshTurso.DataLayer.Info.unique_index_names()
      |> case do
        {m, f, a} -> List.wrap(apply(m, f, [changeset | a]))
        value -> List.wrap(value)
      end
  ...
  defp conflict_target(resource, keys) do
    if Ash.Resource.Info.base_filter(resource) do
      base_filter_sql =
        AshTurso.DataLayer.Info.base_filter_sql(resource) ||
          raise """
          Cannot use upserts with resources that have a base_filter without also adding `base_filter_sql` in the turso section.
          """
  ...
  def destroy(resource, %{data: record} = changeset) do
    ...
    repo = AshSql.dynamic_repo(resource, AshTurso.SqlImplementation, changeset)
  ...
  def select(query, select, resource) do
    query = AshSql.Bindings.default_bindings(query, resource, AshTurso.SqlImplementation)
  ...
  def rollback(resource, term) do
    AshTurso.DataLayer.Info.repo(resource, :mutate).rollback(term)
  end

  defp table(resource, changeset) do
    changeset.context[:data_layer][:table] || AshTurso.DataLayer.Info.table(resource)
  end
  ...
  defp dynamic_repo(resource, %{__ash_bindings__: %{context: %{data_layer: %{repo: repo}}}}) do
    repo || AshTurso.DataLayer.Info.repo(resource, :read)
  end
  ...
  defp dynamic_repo(resource, _) do
    AshTurso.DataLayer.Info.repo(resource, :read)
  end
