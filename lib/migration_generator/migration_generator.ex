...
defmodule AshTurso.MigrationGenerator do
  @moduledoc false

  require Logger

  alias AshTurso.MigrationGenerator.{Operation, Phase}
  ...
  def generate(domains, opts \\ []) do
    ...
    snapshots =
      ...
      |> Enum.filter(fn resource ->
        Ash.DataLayer.data_layer(resource) == AshTurso.DataLayer &&
          AshTurso.DataLayer.Info.migrate?(resource)
      end)
  ...
  def take_snapshots(domain, repo, only_resources \\ nil) do
    ...
    |> Enum.filter(fn resource ->
      Ash.DataLayer.data_layer(resource) == AshTurso.DataLayer &&
        AshTurso.DataLayer.Info.repo(resource) == repo &&
    ...
    for repo <- repos do
      ...
      if Enum.empty?(to_install) do
        ...
        defmodule #{inspect(module_name)} do
          ...
          This file was autogenerated with `mix ash_turso.generate_migrations`
          \"\"\"
  ...
  defp require_name!(opts) do
    if !opts.name && !opts.dry_run && !opts.check && !opts.dev && !opts.auto_name do
      ...
      Please provide a name. for example:
          ...
          mix ash_turso.generate_migrations <name> ...args
      """
    end
  ...
  defp add_custom_index_name(custom_index, table) do
    ...
    |> Map.put_new_lazy(:name, fn ->
      AshTurso.CustomIndex.name(table, %{fields: custom_index.fields})
    end)
    |> Map.update!(
      :name,
      &(&1 || AshTurso.CustomIndex.name(table, %{fields: custom_index.fields}))
    )
  end
  ...
  defp timestamp do
    ...
    last = Process.get(:ash_turso_last_migration_timestamp)
    ...
    Process.put(:ash_turso_last_migration_timestamp, result)
    result
  end
  ...
  def get_snapshots(resource, all_resources) do
    Code.ensure_compiled!(AshTurso.DataLayer.Info.repo(resource))

    if AshTurso.DataLayer.Info.polymorphic?(resource) do
      all_resources
      ...
      |> Enum.map(fn relationship ->
        ...
        |> Map.update!(:identities, fn identities ->
          identity_index_names = AshTurso.DataLayer.Info.identity_index_names(resource)
              ...
              Map.put(attribute, :references, %{
                ...
                destination_attribute_default:
                  default(
                    source_attribute,
                    relationship.destination,
                    AshTurso.DataLayer.Info.repo(relationship.destination)
                  ),
                deferrable: false,
                destination_attribute_generated: source_attribute.generated?,
                multitenancy: multitenancy(relationship.source),
                table: AshTurso.DataLayer.Info.table(relationship.source),
                on_delete: AshTurso.DataLayer.Info.polymorphic_on_delete(relationship.source),
                on_update: AshTurso.DataLayer.Info.polymorphic_on_update(relationship.source),
                primary_key?: source_attribute.primary_key?,
                name:
                  AshTurso.DataLayer.Info.polymorphic_name(relationship.source) ||
      ...
      [do_snapshot(resource, AshTurso.DataLayer.Info.table(resource))]
    end
  end
  ...
  defp do_snapshot(resource, table) do
    snapshot = %{
      ...
      table: table || AshTurso.DataLayer.Info.table(resource),
      custom_indexes: custom_indexes(resource),
      custom_statements: custom_statements(resource),
      repo: AshTurso.DataLayer.Info.repo(resource),
      multitenancy: multitenancy(resource),
      base_filter: AshTurso.DataLayer.Info.base_filter_sql(resource),
      has_create_action: has_create_action?(resource),
      strict?: AshTurso.DataLayer.Info.strict?(resource)
    }
  ...
  defp custom_indexes(resource) do
    resource
    |> AshTurso.DataLayer.Info.custom_indexes()
    |> Enum.map(fn custom_index ->
      Map.take(custom_index, AshTurso.CustomIndex.fields())
    end)
  end

  defp custom_statements(resource) do
    resource
    |> AshTurso.DataLayer.Info.custom_statements()
    |> Enum.map(fn custom_statement ->
      Map.take(custom_statement, AshTurso.Statement.fields())
    end)
  end
  ...
  defp attributes(resource, table) do
    repo = AshTurso.DataLayer.Info.repo(resource)
    ignored = AshTurso.DataLayer.Info.migration_ignore_attributes(resource) || []
    ...
    |> Enum.map(fn attribute ->
      ...
      type =
        AshTurso.DataLayer.Info.migration_types(resource)[attribute.name] ||
          ...
          %{
            ...
            table:
              relationship.context[:data_layer][:table] ||
                AshTurso.DataLayer.Info.table(relationship.destination)
          }
        end
      end
    end)
  ...
  defp foreign_key?(relationship) do
    Ash.DataLayer.data_layer(relationship.source) == AshTurso.DataLayer &&
      AshTurso.DataLayer.Info.repo(relationship.source) ==
        AshTurso.DataLayer.Info.repo(relationship.destination)
  end
  ...
  defp identities(resource) do
    ...
    |> case do
      ...
      identities ->
        ...
        if base_filter && !AshTurso.DataLayer.Info.base_filter_sql(resource) do
          raise """
    ...
    |> Enum.reject(fn identity ->
      identity.name in AshTurso.DataLayer.Info.skip_unique_indexes(resource)
    end)
    ...
    |> Enum.map(fn identity ->
      Map.put(
        ...
        :index_name,
        identity_index_names[identity.name] ||
          "#{AshTurso.DataLayer.Info.table(resource)}_#{identity.name}_index"
      )
    end)
    |> Enum.map(&Map.put_new(&1, :nils_distinct?, true))
    |> Enum.map(&Map.put(&1, :base_filter, AshTurso.DataLayer.Info.base_filter_sql(resource)))
  end
  ...
  defp migration_default(type, constraints, value) do
    ...
    if function_exported?(type, :value_to_turso_default, 3) do
      type.value_to_turso_default(type, constraints, value)
    else
      :error
    end
  end
  ...
  defp configured_default(resource, attribute) do
    AshTurso.DataLayer.Info.migration_defaults(resource)[attribute]
  end
