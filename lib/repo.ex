# SPDX-FileCopyrightText: 2024 AshTurso contributors
#
# SPDX-License-Identifier: MIT

defmodule AshTurso.Repo do
  @moduledoc """
  Resources that use `AshTurso.DataLayer` use a `Repo` to access the database.

  This repo is a thin wrapper around an `Ecto.Repo`, configured to use the
  `Ecto.Adapters.LibSql` adapter for Turso/libSQL connectivity.

  ## Connection Modes

  AshTurso supports three connection modes, configured via your repo's config:
  ...
  Because an `AshTurso.Repo` is also an `Ecto.Repo`, it has all of the same callbacks.

  In the `c:Ecto.Repo.init/2` callback, you can configure the following additional items:

  - `:tenant_migrations_path` - The path where your tenant migrations are stored
  - `:snapshots_path` - The path where the resource snapshots for the migration generator are stored
  - `:encryption_key` - An encryption key for the local database (minimum 32 characters)
  """
  ...
  @doc """
  Use this to inform the data layer about the minimum SQLite version it will be run on.

  libSQL is a fork of SQLite, so SQLite version constraints still apply.
