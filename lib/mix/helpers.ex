...
defmodule AshTurso.Mix.Helpers do
  @moduledoc false
  ...
  def repos!(opts, args) do
    if opts[:domains] && opts[:domains] != "" do
      ...
      resources =
        ...
        |> Enum.filter(&(Ash.DataLayer.data_layer(&1) == AshTurso.DataLayer))
        |> case do
          [] ->
            raise """
            No resources with `data_layer: AshTurso.DataLayer` found in the domains #{Enum.map_join(domains, ",", &inspect/1)}.

            Must be able to find at least one resource with `data_layer: AshTurso.DataLayer`.
            """
      ...
      resources
      |> Enum.map(&AshTurso.DataLayer.Info.repo/1)
      |> Enum.uniq()
      |> case do
        [] ->
          raise """
          ...
          The following resources were found with `data_layer: AshTurso.DataLayer`:
      ...
      |> Enum.filter(fn repo ->
        Spark.implements_behaviour?(repo, AshTurso.Repo)
      end)
    end
  end
